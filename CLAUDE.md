# Coding Agent — Istruzioni per Claude Code
...
| LLM proxy | LiteLLM su `http://localhost:4000` |
...
```
agents_crew/
├── CLAUDE.md              ← questo file (istruzioni agente)
├── MEMORY.md              ← seed knowledge graph al bootstrap
├── Makefile               ← up / bootstrap / claude / models
├── .env.example           ← copia in .env e compila
├── .claude/
│   └── settings.json      ← MCP config per Claude Code
├── litellm/
...
ollama pull qwen2.5-coder:14b  # code generation (~9GB) — via LiteLLM
```

## Quick reference comandi

```bash
make up          # avvia stack Docker (litellm + neo4j + memory)
make bootstrap   # popola knowledge graph dal codebase (solo prima volta)
make claude      # lancia Claude Code puntato a LiteLLM
make models      # pull tutti i modelli Ollama necessari
